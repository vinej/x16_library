<#
.SYNOPSIS
    Assemble an x16lib (vasm edition) program and optionally run it.

    The vasm tree is generated from src_acme/ by tools/acme2vasm.py and
    mirrors it file for file; the same on-target test suite proves the
    two behave identically (and every example assembles byte-for-byte
    identically to the ACME, ca65, 64tass, KickAssembler, dasm and MADS
    builds).

.EXAMPLE
    .\build_vasm.ps1 -Test               # the regression suite, headless
    .\build_vasm.ps1 -Source examples\bounce.asm -Run
#>
param(
    [string]$Source = "test_vasm\runner.asm",
    [switch]$Test,
    [switch]$Run,
    [int]$Scale = 2
)
...
$src   = Join-Path $root "src_vasm"
$build = Join-Path $root "build"

$vasm = Join-Path $root "vasm\vasm6502_oldstyle.exe"
if (-not (Test-Path $vasm)) { Fail "missing: $vasm (put vasm6502_oldstyle.exe there -- see README)" }
...
$out  = Join-Path $build "$name-VASM.PRG"

# vasm resolves `include` relative to the current dir and each -I path;
# src_vasm/ carries the library, $root carries test_vasm/testlib.asm.
# -c02 selects the 65C02 instruction set (trb/tsb, bra, stz, ...);
# -Fbin -cbm-prg writes a CBM .prg (2-byte load address + image).
Write-Host "vasm  $Source -> $out"
Push-Location $root
try {
    & $vasm -c02 -Fbin -cbm-prg -I $src -I $root -o $out $Source
} finally {
    Pop-Location
}
if ($LASTEXITCODE -ne 0) { Fail "vasm assembly failed" }

$size = (Get-Item $out).Length
Write-Host "      $size bytes"

# --- test (identical harness to build_acme.ps1 / build_64tass.ps1) -----
if ($Test) {
    Write-Host "x16emu (headless testbench)"

    $stdin  = Join-Path $env:TEMP "x16lib-empty.in"
    $stdout = Join-Path $build "test-vasm-output.txt"
