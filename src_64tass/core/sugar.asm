...
.if xuse_screen
xm_screen_scroll .macro top, left, height, width, rows, dir
    lda #\top
    sta X16_P0
    lda #\left
    sta X16_P1
...
.if xuse_screen
xm_screen_blit .macro addr, count, col
    lda #<(\addr)
    sta X16_P0
    lda #>(\addr)
    sta X16_P1
...
.if xuse_screen
xm_screen_blitfill .macro count, col, ch
    lda #\count
    ldx #\col
    ldy #\ch
    jsr screen_blitfill
...
.if xuse_verafx
xm_fx_fill .macro val, addr, count
    lda #<(\addr)
    sta X16_P0
    lda #>(\addr)
    sta X16_P1
...
.if xuse_dir
xm_dir_open .macro path, len, device
    lda #<(\path)
    sta X16_P0
    lda #>(\path)
    sta X16_P1
...
.if xuse_filepick
xm_fp_copy_path .macro dest, size
    lda #<(\dest)
    sta X16_P0
    lda #>(\dest)
    sta X16_P1
...
; -> A = characters copied
.if xuse_filepick
xm_fp_copy_name .macro dest, size
    lda #<(\dest)
    sta X16_P0
    lda #>(\dest)
    sta X16_P1
...
; -> A = characters copied
.if xuse_filepick
xm_fp_copy_dir .macro dest, size
    lda #<(\dest)
    sta X16_P0
    lda #>(\dest)
    sta X16_P1
...
.if xuse_stack
xm_stack_isempty .macro
    jsr stack_isempty
    .endm
.endif

.if xuse_stack
xm_stack_isfull .macro
    jsr stack_isfull
    .endm
.endif
...
.if xuse_ringbuffer
xm_ring_isempty .macro
    jsr ring_isempty
    .endm
.endif

.if xuse_ringbuffer
xm_ring_isfull .macro
    jsr ring_isfull
    .endm
.endif
...
.if xuse_irq_any
xm_irq_line_remove .macro
    jsr irq_line_remove
    .endm
.endif

.if xuse_irq_any
xm_irq_save_regs .macro
    jsr irq_save_regs
    .endm
.endif

.if xuse_irq_any
xm_irq_restore_regs .macro
...
xm_screen_get_mode .macro
    jsr screen_get_mode
    .endm
.endif

.if xuse_screen_extra
xm_screen_get_cursor .macro
    jsr screen_get_cursor
    .endm
.endif

.if xuse_vera_fxprobe
xm_vera_has_fx .macro
    jsr vera_has_fx
    .endm
.endif
...
.if xuse_tile
xm_layer_set_tilebase .macro layer, base
    ldx #\layer
    lda #\base
...
.if xuse_bits
xm_bit_put .macro addr, mask, set
    lda #<(\addr)
    sta X16_PTR0
    lda #>(\addr)
    sta X16_PTR0+1
...
.if xuse_ym
xm_ym_patch_ram .macro channel, addr
    clc
    ldx #<(\addr)
    ldy #>(\addr)
    lda #\channel
...
.if xuse_dos
xm_dos_rename .macro newname, newlen, oldname, oldlen
    lda #<(\oldname)
    sta X16_P0
    lda #>(\oldname)
    sta X16_P1
...
.if xuse_bank
xm_bank_to_mem .macro bank, offset, dst, count
    lda #<(\offset)
    sta X16_P1
    lda #>(\offset)
    sta X16_P2
...
.if xuse_bank
xm_bank_copy_far .macro srcbank, srcoff, dstbank, dstoff, count
    lda #<(\srcoff)
    sta X16_P1
    lda #>(\srcoff)
    sta X16_P2
...
.if xuse_load
xm_fs_save .macro name, len, device, start, end
    lda #<(\name)
    sta X16_P0
    lda #>(\name)
    sta X16_P1
...
.if xuse_bmx
xm_bmx_save .macro name, len, device, vbank, vaddr
    lda #<(\name)
    sta X16_P0
    lda #>(\name)
    sta X16_P1
...
.if xuse_pcm_stream
xm_pcm_stream_start_bank .macro offset, count, counthi, bank, rate
    lda #<(\offset)
    sta X16_P0
    lda #>(\offset)
    sta X16_P1
...
.if xuse_verafx_copy
xm_fx_copy .macro src, srchi, dst, dsthi, count
    lda #<(\src)
    sta X16_P0
    lda #>(\src)
    sta X16_P1
...
xm_fx_affine_on .macro tiledata, tiledatahi, tilemap, tilemaphi, mapsize, clip
    lda #<(\tiledata)
    sta X16_P0
    lda #>(\tiledata)
    sta X16_P1
...
.if xuse_verafx_affine
xm_fx_affine_ray .macro x, y, dx, dy
    lda #<(\x)
    sta X16_P0
    lda #>(\x)
    sta X16_P1
