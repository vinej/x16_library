...
test_fill_zero
    ...
    ldy #>_name
    jmp t_result
_name .text "VERA_FILL_ZERO_COUNT", $00
...
test_dcsel_preserves_addrsel
    ...
    beq _fail                   ; ADDRSEL got cleared -> bug

    lda VERA_CTRL
    and #VERA_CTRL_DCSEL
    cmp #(2 << 1)
    bne _fail                   ; DCSEL didn't take
...
_report
    ...
    ldy #>_name
    jmp t_result
_name .text "DCSEL_KEEPS_ADDRSEL", $00
...
test_collide_overlap
    ...
    jsr collide8
    lda #0
    bcs _report                 ; carry set = overlap = pass
    lda #1
_report
    ldx #<_name
    ldy #>_name
    jmp t_result
_name .text "COLLIDE_OVERLAP", $00
...
test_collide_apart
    ...
    jsr collide8
    lda #0
    bcc _report                 ; carry clear = no overlap = pass
    lda #1
_report
    ldx #<_name
    ldy #>_name
    jmp t_result
_name .text "COLLIDE_APART", $00
...
_report
    ...
    ldy #>_name
    jmp t_result
_name .text "COLLIDE_TOUCHING", $00
...
test_collide16
    ...
    #set16 cl_by, 200
    jsr collide16
    bcs _fail_far               ; touching is not overlapping
    bra _deeper

_fail_far                       ; _fail is out of branch range from here
    jmp _fail
...
_inner
    jsr irq_frames
    sec
    sbc _start                  ; byte subtraction wraps correctly
    cmp #2
    bcs _ok                     ; two frames elapsed
    dex
    bne _inner
    dey
    bne _outer
    ...
    sec
    sbc _jiffy0
    beq _skip                   ; jiffy frozen: this machine has no VSYNC
    jsr irq_remove
...
test_bank_roundtrip
    ...
    lda RAM_BANK
    cmp #3
    bne _fail                   ; caller's bank was not restored
...
_report
    ...
    ldy #>_name
    jmp t_result
_src   .byte $10, $21, $32, $43, $54, $65, $76, $87
_dst   .fill 8, 0
_saved .byte 0
_name  .text "BANK_ROUNDTRIP", $00
...
_report
    ...
    ldy #>_name
    jmp t_result
_pat   .byte $AA, $BB, $CC, $DD
_saved .byte 0
_name  .text "BANK_BOUNDARY", $00
...
test_gfx_clip
    ...
    lda VERA_DATA1
    cmp #$11
    bne _fail                   ; the clip did not hold
...
test_ym_write
    ...
    jsr ym_write
    bcs _fail                   ; carry set = the chip stayed busy
...
test_ym_channel_in_a
    jsr ym_init
    bcs _skip                   ; no YM2151 on this machine
    ...
    jsr ym_get_pan
    cpx #2
    bne _fail                   ; the pan did not reach channel 5

    lda #2                      ; the channel the swapped call would hit
    jsr ym_get_pan
    cpx #2
    beq _fail                   ; ...and it must not have landed there
...
test_bits
    ...
    lda #%00000100
    jsr bit_test
    beq _fail                   ; the bit is set, so Z must be clear
    lda #%00001000
    jsr bit_test
    bne _fail                   ; that bit is clear
...
_vs8
    ...
    bra _cont                   ; fail/report kept mid-routine for branch range
_fail
    lda #1
    bra _report
_ok
...
_src_u8  .byte 5, 3, 8, 1, 9, 2
_exp_u8  .byte 1, 2, 3, 5, 8, 9
_src_s8  .byte $FB, $03, $80, $7F, $00      ; -5, 3, -128, 127, 0
_exp_s8  .byte $80, $FB, $00, $03, $7F      ; -128, -5, 0, 3, 127
_src_u16 .word $0300, $0100, $0200
_exp_u16 .word $0100, $0200, $0300
_src_s16 .word $FFFF, $0001, $8000, $7FFF   ; -1, 1, -32768, 32767
_exp_s16 .word $8000, $FFFF, $0001, $7FFF
_buf     .fill 16, 0
_name    .text "SORT", $00
...
test_number_parse
    ...
    lda #<_bad                  ; a non-digit must be rejected
    sta X16_P0
    lda #>_bad
    sta X16_P1
    lda #4
...
; every comparison matched. A branching version puts _fail out of range
; after a few expansions -- and chk_err is already exactly the convention
; t_result wants (0 = pass).
; =====================================================================
chk_err .byte 0

chk16 .macro addr, value
    lda \addr
    eor #<(\value)
    ora chk_err
    sta chk_err
...
test_i16_cmp
    ...
    jsr i16_cmpu
    cmp #1
    bne _fail                   ; unsigned: a > b

    jsr i16_cmps
    cmp #$FF
    bne _fail                   ; signed: a is -1, so a < b

    #i16_const i16_a, 12345
    #i16_const i16_b, 12345
    jsr i16_cmps
    bne _fail                   ; Z set on equality
    cmp #0
    bne _fail
...
_report
    ...
    ldy #>_name
    jmp t_result
_max  .text "4294967295", $00
_zero .text "0", $00
_mil  .text "1000000", $00
_name .text "I32_TO_DEC", $00
...
_report
    ...
    ldy #>_name
    jmp t_result
_three .fill FP_SIZE, 0
_name  .text "F_SUB_ORDER", $00
...
_report
    ...
    ldy #>_name
    jmp t_result
_four    .fill FP_SIZE, 0
_hundred .fill FP_SIZE, 0
_expect  .text ".04", $00
_name    .text "F_DIV_ORDER", $00
...
test_fs_roundtrip
    ...
    lda #>_src
    sta X16_P6
    lda #<(_src + 8)            ; end address, exclusive
    sta X16_T6
    lda #>(_src + 8)
    sta X16_T7
    jsr fs_save
...
_fname_len = 12
_src       .byte $DE, $AD, $BE, $EF, $CA, $FE, $BA, $BE
_dst       .fill 8, 0
_name      .text "FS_ROUNDTRIP", $00
...
_loop
    ...
    bne _bad
    cmp #0
    beq _ok                     ; both terminated at the same place
    iny
    bne _loop
_bad
    sec
...
; so long tests never push _fail out of branch range.
chkv .macro expect
    lda VERA_DATA1
    eor #\expect
    ora chk_err
...
test_pal_load
    ...
    ldy #>_name
    jmp t_result
_colors .byte $23, $01, $56, $04, $89, $07   ; $0123, $0456, $0789
_name   .text "PAL_LOAD", $00
...
test_gfx_clear_full
    ...
    ldy #>_name
    jmp t_result
_name .text "GFX8L_CLEAR_FULL", $00
...
test_gfx_hline_long
    ...
    ldy #>_name
    jmp t_result
_name .text "GFX8L_HLINE_LONG", $00
...
test_gfx_line_steep
    ...
    ldy #>_name
    jmp t_result
_name .text "GFX8L_LINE_STEEP", $00
...
test_collide8_9bit
    ...
    sta X16_P7
    jsr collide8
    bcc _fail                   ; they overlap on both axes
    ...
    sta X16_P7
    jsr collide8
    bcs _fail                   ; touching is not overlapping
...
_report
    ...
    ldy #>_name
    jmp t_result
_expect .text "000A", $00
_name   .text "NUMBER_HEX_LOW", $00
...
test_number_parse_edge
    ...
    lda #<_zero                 ; length 0: nothing to reject, value 0
    sta X16_P0
    lda #>_zero
    sta X16_P1
    stz X16_P2
...
_max  .text "65535"
_zero .text "0"
_name .text "NUMBER_PARSE_EDGE", $00
...
_report
    ...
    ldy #>_name
    jmp t_result
_five .fill FP_SIZE, 0
_name .text "F_NEG_CMP", $00
...
test_pcm_fifo
    ...
    jsr pcm_empty
    bcc _fail                   ; must start empty

    lda #$55
    jsr pcm_put
    jsr pcm_empty
    bcs _fail                   ; one byte queued: not empty
    jsr pcm_full
    bcs _fail                   ; ...and nowhere near full

    jsr pcm_reset
    jsr pcm_empty
    bcc _fail                   ; the reset must drain it
...
_drain
    ...
    dex
    bne _drain
    bra _fail                   ; the buffer never emptied
_empty
    jsr key_peek
    bne _fail                   ; Z must be set when empty
    cpx #0
    bne _fail
...
test_fs_missing
    ...
    sta X16_P6
    jsr fs_load
    bcc _fail                   ; it "loaded" a missing file
...
test_screen_mode_rt
    ...
    bra _report
_fail
    lda _saved                  ; put the screen back even on the way out
    jsr screen_set_mode
    lda #1
_report
    ldx #<_name
...
_poison
    ...
    lda _buf                    ; the byte before must survive
    bne _fail
    lda _buf+41                 ; ...and the byte after
    bne _fail
    ldx #40
_check
    lda _buf,x
...
_report
    ...
    ldy #>_name
    jmp t_result
_src  .byte $91, $82, $73, $64, $55, $46, $37, $28
_dst  .fill 8, 0
_dst2 .fill 8, 0
_name .text "MEM_COPY", $00
...
_report
    ...
    ldy #>_name
    jmp t_result
_check .text "123456789"
_name  .text "MEM_CRC", $00
...
test_mem_decompress
    ...
    sta X16_P3
    jsr mem_decompress
    cmp #<(_out+96)             ; A/X = one past the last byte
    bne _fail_far
    cpx #>(_out+96)
    bne _fail_far
    lda _out+96
    ...
    lda #<_out                  ; the payload is the 24-byte phrase x4
    sta T_ZP
    lda #>_out
    sta T_ZP+1
    ldx #4
...
_report
    ...
    ldy #>_name
    jmp t_result
_expect .text "X16LIB-DECOMPRESS-TEST!!"
_packed                         ; lzsa -r -f2 of that phrase repeated 4x
    .byte $3f, $f4, $06, $58, $31, $36, $4c, $49, $42, $2d, $44, $45
    .byte $43, $4f, $4d, $50, $52, $45, $53, $53, $2d, $54, $45, $53
    .byte $54, $21, $21, $ff, $30, $e7, $e8
_out    .fill 97, 0
_name   .text "MEM_DECOMPRESS", $00
...
test_bank_alloc
    ...
    bne _fail
    jsr bank_alloc
    bcc _fail                   ; the pool is empty now
...
_inner
    ...
    sec
    sbc _jiffy0
    beq _skip                   ; no interrupts at all on this machine
    jsr irq_line_remove
    jsr irq_remove
    lda #1
    bra _report
...
test_pcm_stream
    ...
    jsr pcm_full
    bcc _fail                   ; the FIFO was primed to the brim
    jsr pcm_stream_active
    beq _fail                   ; 1 KB still waiting
    lda VERA_IEN
    and #VERA_IRQ_AFLOW
    beq _fail                   ; ...so the refill IRQ is armed
    ...
    jsr pcm_stream_active
    bne _fail                   ; everything was handed over at start
    lda VERA_IEN
    and #VERA_IRQ_AFLOW
    bne _fail                   ; ...so no refill IRQ is needed
    jsr pcm_empty
    bcs _fail                   ; but the 64 bytes are queued
...
test_irq_remove_aflow
    ...
    lda VERA_IEN
    and #VERA_IRQ_AFLOW
    beq _fail                   ; precondition: the stream armed AFLOW

    jsr irq_remove              ; unhook mid-stream, like a rude caller

    lda VERA_IEN
    and #VERA_IRQ_AFLOW
    bne _fail                   ; AFLOW must be gone with the hook
    jsr pcm_stream_active
    bne _fail                   ; and the stream must know it is dead
...
_report
    ...
    ldy #>_name
    jmp t_result
_name .text "IRQ_REMOVE_AFLOW", $00
...
_vol2                           ; A = voice 2's volume/pan shadow byte
    #vera_addr 1, VRAM_PSG + (2*4) + 2, VERA_INC_1
    lda VERA_DATA1
    rts
_name .text "PSG_ENV", $00
...
test_buffers
    jsr rb_init
    jsr rb_get
    bcc _fail                   ; empty reads carry set
...
_wrap
    ...
    bne _fail
    inc _i
    bne _wrap                   ; 256 rounds = 512 puts: plenty of wrap
    jsr rb_count
    bne _fail                   ; balanced: empty again
...
_report
    ...
    ldy #>_name
    jmp t_result
_packed .byte $17, $28, $93, $4C, $E5, $0A, $71, $BF
_expect                         ; audioop's 16 little-endian samples
...
; =====================================================================
test_dos
    lda #<_badcmd               ; nonsense command: an error class code
    ldx #>_badcmd
    ldy #2
    jsr dos_cmd
    bcc _fail_far               ; must be >= 20
    jsr dos_status              ; reading it cleared it
    bcs _fail_far

    lda #<_dirname              ; make a directory...
    ldx #>_dirname
    ldy #4
    jsr dos_mkdir
    bcs _fail_far
...
_files
    ...
    lda #8
    sta X16_P3
    lda #<_fname                ; any 8 bytes will do as content
    sta X16_P5
    lda #>_fname
    sta X16_P6
    lda #<(_fname + 8)
    ...
    lda #<_fname                ; old name in the parameter block
    sta X16_P0
    lda #>_fname
    sta X16_P1
    lda #11
    sta X16_P2
    lda #<_fname2               ; new name in A/X/Y
    ldx #>_fname2
    ldy #11
    jsr dos_rename
    bcs _fail
    ...
    lda #<_dirname              ; leave the fsroot as we found it
    ldx #>_dirname
    ldy #4
    jsr dos_rmdir
    bcs _fail
...
_badcmd  .text "X9"
_dirname .text "TDIR"
_fname   .text "TESTDOS.BIN"
_fname2  .text "TESTREN.BIN"
_name    .text "DOS", $00
...
_row    .byte 0
_fname  .text "TEST.BMX"
_fname2 .text "JUNK.BMX9"
_junk   .byte $DE, $AD, $DE, $AD, $DE, $AD, $DE, $AD
...
test_bmx_missing
    ...
    #cset16 X16_P5, TESTVRAM
    jsr bmx_load
    bcc _fail                   ; it "loaded" a file that does not exist
    cmp #BMX_ERR_IO
    bne _fail
...
_fname     .text "NOSUCH.BMX"
_fname_len = 10
_name      .text "BMX_MISSING", $00
...
test_bmx_truncated
    ...
    #cset16 X16_P5, TESTVRAM
    jsr bmx_load
    bcc _fail_del               ; it "loaded" an image that is not all there
    cmp #BMX_ERR_IO
    bne _fail_del
...
_fname     .text "TRUNC.BMX"
_fname_len = 9
_name      .text "BMX_TRUNCATED", $00
...
test_bmx_short_pal
    ...
    #cset16 X16_P5, TESTVRAM
    jsr bmx_load
    bcc _fail_del               ; it "loaded" a palette that is not there
    cmp #BMX_ERR_IO
    bne _fail_del
...
_fname     .text "SHORTPAL.BMX"
_fname_len = 12
_name      .text "BMX_SHORT_PAL", $00
...
test_zx0
    ...
    lda #<_out                  ; the payload: the 24-byte phrase x4
    sta T_ZP
    lda #>_out
    sta T_ZP+1
    ldx #4
...
_report
    ...
    jmp t_result
_expect .text "X16LIB-DECOMPRESS-TEST!!"
_packed                         ; salvador payload.bin payload.zx0
...
test_tsc
    ...
    lda #<_out                  ; the payload phrase, four times
    sta T_ZP
    lda #>_out
    sta T_ZP+1
    ldx #4
...
_report
    ...
    jmp t_result
_expect .text "X16LIB-DECOMPRESS-TEST!!"
_redge  .text "RLE-EDGE"
_packed                         ; tscrunch payload.bin payload.tsc
    .byte $3f, $19, $58, $31, $36, $4c, $49, $42, $2d, $44, $45, $43
    .byte $4f, $4d, $50, $52, $45, $53, $53, $2d, $54, $45, $53, $54
    .byte $21, $21, $58, $fe, $18, $cc, $e8, $7f, $20
_rpacked                        ; tscrunch rle.bin rle.tsc
...
test_pcm_stream_loop
    ...
    jsr pcm_full
    bcc _fail                   ; primed to the brim across 40 wraps
    jsr pcm_stream_active
    beq _fail                   ; a looping stream never runs dry
    lda VERA_IEN
    and #VERA_IRQ_AFLOW
    beq _fail
