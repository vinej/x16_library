...
.label PCM_FIFO_FULL = %10000000
.label PCM_FIFO_EMPTY = %01000000
.label PCM_FIFO_RESET = %10000000     // on write
.label PCM_16BIT = %00100000
.label PCM_STEREO = %00010000
...
pcm_write__loop:
    lda X16_P2
    ora X16_P3
    beq pcm_write__done                   // count exhausted
...
pcm_start_common:
    ...
    beq pcm_stream_start_bank__go                     // no: it all fit in the FIFO
    php
    sei
    lda #VERA_IRQ_AFLOW
    tsb VERA_IEN
    plp
pcm_stream_start_bank__go:
    pla
...
pcm_stream_fill:
    lda pcm_str_mode
    beq pcm_stream_fill__loop
    lda RAM_BANK                // banked source: map it in, and put the
    sta pcm_str_svbk            // interrupted code's bank back on exit
    lda pcm_str_bank
    sta RAM_BANK
pcm_stream_fill__loop:
    lda pcm_str_rem
    ora pcm_str_rem+1
    ora pcm_str_rem+2
    beq pcm_stream_fill__exhausted
    bit VERA_AUDIO_CTRL         // bit 7: FIFO full
    bpl pcm_stream_fill__feed
    jmp pcm_stream_fill__full                   // out of branch range from here
pcm_stream_fill__feed:
pcm_src:
    lda $FFFF                   // operand = current source (self-modified)
    sta VERA_AUDIO_DATA

    inc pcm_src+1                  // advance the source
    bne pcm_stream_fill__dec
    inc pcm_src+2
    lda pcm_str_mode
    beq pcm_stream_fill__dec
    ...
    lda pcm_str_bank
    sta RAM_BANK
pcm_stream_fill__dec:
    lda pcm_str_rem             // 24-bit decrement
    bne pcm_stream_fill__dec0
    lda pcm_str_rem+1
    bne pcm_stream_fill__dec1
    dec pcm_str_rem+2
pcm_stream_fill__dec1:
    dec pcm_str_rem+1
pcm_stream_fill__dec0:
    dec pcm_str_rem
    bra pcm_stream_fill__loop

pcm_stream_fill__exhausted:
    lda pcm_str_loop
    beq pcm_stream_fill__stop_refill
    ...
    sta pcm_str_rem+2
    lda pcm_str_mode
    bne pcm_stream_fill__rewind_bank
    jmp pcm_stream_fill__loop                   // pcm_stream_fill__loop is out of branch range from here
pcm_stream_fill__rewind_bank:
    lda pcm_str_rbank           // ...including the starting bank
    sta pcm_str_bank
    sta RAM_BANK
    jmp pcm_stream_fill__loop

pcm_stream_fill__stop_refill:
...
pcm_stream_fill__full:
    lda pcm_str_mode
    beq pcm_stream_fill__out
    lda pcm_str_svbk            // the interrupted code's bank goes back
    sta RAM_BANK
pcm_stream_fill__out:
    rts
