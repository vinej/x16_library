...
//
// output_frequency = 25000000/512 / 2((17) >> 16) * freq_word
...
.label PSG_PAN_LEFT = %01000000
.label PSG_PAN_RIGHT = %10000000
.label PSG_PAN_BOTH = %11000000

.label PSG_WAVE_PULSE = %00000000
.label PSG_WAVE_SAWTOOTH = %01000000
.label PSG_WAVE_TRIANGLE = %10000000
.label PSG_WAVE_NOISE = %11000000
...
psg_env_start:
    ...
    sta env_rstep,x
    lda X16_P1
    beq psg_env_start__instant
    stz env_vol,x
    lda #1                      // stage 1: attack
    sta env_stage,x
    rts
psg_env_start__instant:
    lda env_peak,x
    sta env_vol,x
...
psg_env_release:
    ...
    tax
    lda env_stage,x
    beq psg_env_release__done                   // not playing
    lda #3
    sta env_stage,x
psg_env_release__done:
    rts
...
psg_env_tick__voice:
    lda env_stage,x
    beq psg_env_tick__next                   // 0: idle
    cmp #2
    beq psg_env_tick__sustain
    bcc psg_env_tick__attack                 // 1

    // --- release ---
    lda env_rstep,x
    beq psg_env_tick__next                   // rstep 0: hold until psg_env_stop
    sta X16_T0
    lda env_vol,x
    sec
    sbc X16_T0
    bcs psg_env_tick__rel_ok
    lda #0
psg_env_tick__rel_ok:
    sta env_vol,x
    bne psg_env_tick__write
    stz env_stage,x             // faded out: disarm
    bra psg_env_tick__write

psg_env_tick__attack:
    lda env_vol,x
    clc
    adc env_astep,x
    cmp env_peak,x
    bcc psg_env_tick__att_ok
    lda env_peak,x              // reached (or overshot) the peak
    pha
    lda #2
    sta env_stage,x
    pla
psg_env_tick__att_ok:
    sta env_vol,x
    bra psg_env_tick__write
