...
// because ACME's double__cheap locals do NOT reset at a zone-local routine label
// -- two routines cannot each own an double__loop.
//
...
//          value = (-1)((sign) >> 16) * dac_m * 2((dac_e) >> 16)
...
.label d_ptr = X16_TPTR0       // operand pointer (shared scratch)
.label dstr_ptr = X16_TPTR1       // d_from_str's string pointer (survives the
...
double_dfr_nz:
    // magnitude into the top 32 bits of dac_m (= value * 2((32) >> 16)), then
...
double_dto_oneg:
    ...
    lda #$80
    sta X16_P3
    // -2((31) >> 16) is the one value that lands here and IS representable: it is
    // dac_m = 2((63) >> 16) with shift exactly 32, so it must not be flagged.
    lda d_cnt+1
    bne double_dto_onov
    lda d_cnt
    cmp #32
...
// ---------------------------------------------------------------------
// d_exp -- d_ac = e((d_ac) >> 16)
//
// Range-reduce x = n*ln2 + r (n = trunc(x/ln2), |r| < ln2), sum the
// Taylor series e((r) >> 16) = 1 + r + r((2) >> 16)/2! + ..., then scale by 2((n) >> 16) (add n to
...
double_dex_norm:
    ...
    // end of the range (e((23000) >> 16) came out +0), and past ~1.49e9 d_to_s32
...
double_dex_scale:
    ...
    ldy #>d_tsum
    jsr d_load
    lda #<d_ac                   // multiply by 2((n) >> 16): exponent += n
    sta d_ptr
    lda #>d_ac
    sta d_ptr+1
    jsr double_d_unpack
...
// d_ln -- d_ac = ln(d_ac)
//
// Split value = m * 2((e) >> 16) with m in [0.75, 1.5) (halving m once if >= 1.5),
// so ln = e*ln2 + ln(m); ln(m) = 2*(t + t((3) >> 16)/3 + t((5) >> 16)/5 + ...) with
...
//
// x((y) >> 16) = exp(y * ln x). y == 0 gives 1 (even for x <= 0); otherwise a
...
// then the fast series x - x((3) >> 16)/3 + x((5) >> 16)/5 - ...  +-inf -> +-pi/2.
// ---------------------------------------------------------------------
d_atan:
    lda #<d_ac
    sta d_ptr
...
double_dat_norecip:
    ...
    sta d_atflags
double_dat_nosixth:
    // atan(r) = r - r((3) >> 16)/3 + r((5) >> 16)/5 - ...  Carry the power p = r((2k+1) >> 16) in
    // d_atn (only ever *= -r((2) >> 16), so its sign alternates); each term divides
    // a COPY of p by (2k+1) -- p itself must not be divided.
    lda #<d_tr
    ldy #>d_tr
    jsr d_store                  // r = reduced x
    jsr double_d_trig_nr2              // d_tt = -r((2) >> 16)
    lda #<d_tr
    ldy #>d_tr
    jsr d_load
    lda #<d_atn
...
// Beyond 2((31) >> 16) quadrants (|x| >= about 3.37e9) the quadrant count no longer
...
// sin(d_tr) via Taylor: sum = r, term *= -r((2) >> 16)/((2k)(2k+1)), sum += term
double_d_sinr:
    jsr double_d_trig_nr2              // d_tt = -r((2) >> 16)
    lda #<d_tr
    ldy #>d_tr
    ...
    sta d_tkc
double_dsr_loop:
    jsr double_d_trig_termstep         // term *= -r((2) >> 16)
    lda d_tkc                    // / (2k)
    asl
    ldx #0
...
// cos(d_tr) via Taylor: sum = 1, term *= -r((2) >> 16)/((2k-1)(2k)), sum += term
double_d_cosr:
    jsr double_d_trig_nr2              // d_tt = -r((2) >> 16)
    lda #<d_one
    ldy #>d_one
...
// term = term * (-r((2) >> 16))   [-r((2) >> 16) is in d_tt]
...
double_dstr_edig:
    ...
    sec
    sbc #'0'
    // Stop accumulating at 3 digits: 10((999) >> 16) is already infinity and 10^-999
...
double_dstr_scale:
    ...
    jsr d_neg
double_dstr_ns:
    lda dstr_exp+1               // scale by 10((finalexp) >> 16)
...
d_exphi: .byte $00,$00,$00,$00,$00,$30,$86,$40   // 710.0  (e((x) >> 16) overflows past
                                                 // 709.783, so anything above
                                                 // this is +inf)
d_explo: .byte $00,$00,$00,$00,$00,$50,$87,$C0   // -746.0 (e((x) >> 16) underflows below
