...
collide8:
    ...
    clc
    adc X16_P6                  // bx + bw
    bcs collide8__ax_lt                  // past 255, so ax is certainly less
    cmp X16_P0                  // carry set if (bx+bw) >= ax
    bcc collide8__apart
    beq collide8__apart                  // equal means touching, not overlapping
collide8__ax_lt:
    lda X16_P0
    clc
    adc X16_P2                  // ax + aw
...
collide16:
    ...
    adc cl_bw+1
    sta cl_t1
    bcs collide16__ax_lt                  // sum overflowed 16 bits: ax is less
    lda cl_ax
    cmp cl_t0
    lda cl_ax+1
    sbc cl_t1
    bcs collide16__apart16                // ax >= sum, so touching or clear
collide16__ax_lt:
    ...
    adc cl_aw+1
    sta cl_t1
    bcs collide16__bx_lt
    lda cl_bx
    cmp cl_t0
    lda cl_bx+1
    sbc cl_t1
    bcs collide16__apart16
collide16__bx_lt:
