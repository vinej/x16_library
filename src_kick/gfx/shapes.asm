...
// circle's own shapes_pair4 / shapes_span2. The decision terms reach 2*rx*ry((2) >> 16)
// (about 33M at 255/255), so the arithmetic is 32-bit; the one setup
// product rx * 2ry((2) >> 16) is a repeated subtract, a few thousand cycles at
// the very worst -- noise against the drawing itself.
//   dx = ry((2) >> 16) - rx*2ry((2) >> 16), dy = rx((2) >> 16), err = dx + dy
//   each step: e2 = 2*err;
//     e2 >= dx ?  x++, err += dx += 2ry((2) >> 16)
//     e2 <= dy ?  y++, err += dy += 2rx((2) >> 16)
...
shapes_etake:
	...
	lda shapes_eh                     // shapes_sq = ry((2) >> 16)
	jsr shapes_sq16
	lda shapes_sq                     // dx = ry((2) >> 16) (the rx*2ry((2) >> 16) comes off below)
	sta shapes_edx
	lda shapes_sq+1
	sta shapes_edx+1
	stz shapes_edx+2
	stz shapes_edx+3
	lda shapes_sq                     // shapes_e2b = 2ry((2) >> 16)
	sta shapes_e2b
	lda shapes_sq+1
	sta shapes_e2b+1
	stz shapes_e2b+2
	...
	rol shapes_e2b+1
	rol shapes_e2b+2
	ldx shapes_ew                     // dx -= rx * 2ry((2) >> 16), one 2ry((2) >> 16) at a time
	beq shapes_exset
shapes_emul:
	sec
	lda shapes_edx
	...
	bne shapes_emul
shapes_exset:
	lda shapes_ew                     // shapes_sq = rx((2) >> 16)
	jsr shapes_sq16
	lda shapes_sq                     // dy = rx((2) >> 16)
	sta shapes_edy
	lda shapes_sq+1
	sta shapes_edy+1
	stz shapes_edy+2
	stz shapes_edy+3
	lda shapes_sq                     // shapes_e2a = 2rx((2) >> 16)
	sta shapes_e2a
	lda shapes_sq+1
	sta shapes_e2a+1
	stz shapes_e2a+2
...
// ---------------------------------------------------------------------
.label FLOOD_MAX = 96                  // seeds; 4 bytes each
...
.label POLY_MAX = 24                   // vertices; the buffers below are 2 bytes each
...
.label ARC_STEP = 4                    // byte-angle units between samples
...
#if !SHP_PSET_SET
.label SHP_PSET = gfx2h_pset
#endif
#if !SHP_READ_SET
.label SHP_READ = gfx2h_read
#endif
#if !SHP_HLINE_SET
.label SHP_HLINE = gfx2h_hline
#endif
#if !SHP_W_SET
.label SHP_W = shp_wdef
#endif
#if !SHP_H_SET
.label SHP_H = shp_hdef
#endif
#endif
