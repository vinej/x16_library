...
.label RING_CAP = 8192                 // the bank window is 8192 bytes (0..8191)
...
ring_getw:
    ...
    bne ring_getw__have                   // only answers for a single ring_get, so a
    lda ring_fill               // caller guarding a getw with it walked the
    cmp #2                      // tail past the head on an odd last byte
    bcc ring_getw__under                  // and left the ring corrupt for good
ring_getw__have:
    jsr ringbuffer_filldec
    jsr ringbuffer_filldec
    lda RAM_BANK
