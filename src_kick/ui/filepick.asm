...
.label FPK_NONE = 0                   // cancelled: ESC, Run/Stop, or the x box
.label FPK_PICK = 1                   // a file was chosen: fp_path has it
.label FPK_ALT = 2                   // the second gesture: right click, or 'a'
.label FPK_HERE = 3                   // 'h': this DIRECTORY, not a file in it

.label FPK_ESIZE = 40                  // one cache entry: type, then the name
.label FPK_ETYPE = 0
.label FPK_ENAME = 1
.label FPK_MAXENT = 64
.label FPK_NMSIZE = 40                  // fp_nm: one name, or one being typed
.label FPK_DIRMAX = 63                  // the longest path fp_curdir will keep
.label FPK_FULLSZ = FPK_DIRMAX+1+FPK_ESIZE-2+1
...
.label FPK_NOBANK = 255                 // fp_saveunder: keep nothing
.label FPK_PTOP = 3                   // the panel's first row
.label FPK_DBLCLK = 30                  // jiffies: half a second
.label FPK_ACURSOR = $67                // yellow on blue: the caret, inverse of
                                 // the field it sits in
.label FPK_AEDIT = $76                 // blue on yellow: the one place the panel
...
// ---- state -----------------------------------------------------------
fp_curdir: .fill FPK_DIRMAX+1, 0
fp_full: .fill FPK_FULLSZ, 0
fp_nm: .fill FPK_NMSIZE, 0
fp_nent: .byte 0
fp_sel: .byte 0
fp_top: .byte 0
fp_down: .byte 0
...
filepick_alldef:
    ...
    .byte 0
filepick_footdef:
    .text "double click opens   esc closes"
    .byte 0
filepick_dirtag:
    .text "[dir] "
    .byte 0
...
// =====================================================================
fp_clip: .fill FPK_FULLSZ, 0  // the file 'c' remembered, absolute -- so
                                // it has to be as long as fp_full, or a
                                // deep path is remembered short and the
                                // paste reads from somewhere else
fp_clipok: .byte 0
fp_buf: .fill 256, 0  // what a copy moves at a time
