...
```asm
.cpu _65c02
#import "x16.asm" // constants + macros. Emits no code.

#define X16_USE_SCREEN // pick your modules (or X16_USE_ALL = 1)
...
or define `#define X16_USE_ALL` for the stable all-in bundle. Dependencies resolve themselves
...
- **64tass** — same `#define X16_USE_VERA` spelling as ACME; gates default to 0
  via `.weak`.
...
Defined in `core/macros.asm`, available after `#import "x16.asm"`. Macros
