...
ser_detect:
    ...
    lda VERA2_ID
    cmp #VERA2_ID_MAGIC
    bne ser_detect__noconflict
    lda #<(VERA2_BLIT_CTRL + 1)
    sta X16_T2
    lda #>(VERA2_BLIT_CTRL + 1)
    sta X16_T3
ser_detect__noconflict:
    php
    sei
ser_detect__scan:
    jsr serial_probe
...
ser_read_until__reset:
    ...
    bne ser_read_until__rewind
    lda X16_P7
    cmp X16_T5
    beq ser_read_until__drop
ser_read_until__rewind:
    lda X16_T4
    sta X16_P6
    lda X16_T5
    sta X16_P7
    pla
    bra ser_read_until__cmp
ser_read_until__drop:
    pla
    bra ser_read_until__loop
...
ser_discard_until__loop:
    jsr ser_get_wait
ser_discard_until__test: // re-entered with the byte still in A
    cmp (X16_P6)
...
ser_discard_until__reset:
    ...
    lda X16_P6                  // already at needle[0]? then this byte
    cmp X16_T4                  // really cannot start a match
    bne ser_discard_until__rewind
    lda X16_P7
    cmp X16_T5
    beq ser_discard_until__drop
ser_discard_until__rewind:
    lda X16_T4
    sta X16_P6
    lda X16_T5
    sta X16_P7
    pla
    bra ser_discard_until__test
ser_discard_until__drop:
    pla
    bra ser_discard_until__loop
