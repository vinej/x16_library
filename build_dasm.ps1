<#
.SYNOPSIS
    Assemble an x16lib (dasm edition) program and optionally run it.

    The dasm tree is generated from src_acme/ by tools/acme2dasm.py and
    ...
    identically to the ACME, ca65, KickAssembler and 64tass builds).

.EXAMPLE
    .\build_dasm.ps1 -Test               # the regression suite, headless
    .\build_dasm.ps1 -Source examples\bounce.asm -Run
#>
param(
    [string]$Source = "test_dasm\runner.asm",
    [switch]$Test,
    [switch]$Run,
    [int]$Scale = 2
)
...
if ($Test -and -not $PSBoundParameters.ContainsKey('Source')) {
    foreach ($s in @("test_dasm\runner.asm", "test_dasm\runner2.asm", "test_dasm\serial.asm")) {
...
$dasm = Join-Path $root "dasm\dasm.exe"
if (-not (Test-Path $dasm)) { Fail "missing: $dasm (put dasm.exe there -- see README)" }
...
$out  = Join-Path $build "$name-DASM.PRG"

# dasm resolves `include` relative to the current dir and each -I path;
# src_dasm/ carries the library, $root carries test_dasm/testlib.asm.
# -f1 writes a CBM .prg (2-byte load address + contiguous image).
Write-Host "dasm  $Source -> $out"
Push-Location $root
try {
    & $dasm $Source "-I$src" "-I$root" -f1 "-o$out"
} finally {
    Pop-Location
}
...
if ($Test) {
    ...
    $stdout = Join-Path $build "test-dasm-output.txt"
