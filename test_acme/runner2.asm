...
@report
    ...
    ldy #>@name
    jmp t_result
@stride_lo !byte <64, <128, <256, <512
@stride_hi !byte >64, >128, >256, >512
@name !text "SCREEN_ADDR", $00
...
@report
    ...
    ldy #>@name
    jmp t_result
@text !byte $41, $C2, $30       ; 'a', 'B', '0' in PETSCII
@name !text "SCREEN_BLIT", $00
...
@report
    ...
    ldy #>@name
    jmp t_result
@name !text "G2_PATTERN_PH", $00
@pat2 !byte $F0, $F0, $F0, $F0, $F0, $F0, $F0, $F0
...
@report
    ...
    ldy #>@name
    jmp t_result
@name !text "G2_BLIT", $00
@img !byte $DE, $AD, $BE, $EF
...
@report
    ...
    ldy #>@name
    jmp t_result
@name !text "G2_BLITM", $00
@mcol !byte $0F, $50, $0F, $50, $0F, $50, $0F, $50   ; (mask,data) x4
...
@report
    ...
    ldy #>@name
    jmp t_result
@name !text "G2_INIT", $00
@want !byte $FF, $0F, $AA, $0A, $55, $05, $00, $00
...
; =====================================================================
test_dir_reads
    lda #<@want                 ; lay down the file this test looks for,
    ...
    jsr @entry                  ; the header names the volume
    bcc @fail_close
    jsr dir_type
    cmp #DIR_TYPE_HOST
    bne @fail_close
...
@cmp
    lda @want,x
    cmp @buf,x
    bne @scan                   ; a different file: keep looking
    inx
...
@entry
    ...
    sta X16_P2
    jmp dir_next
@want      !text "TESTDATA.BIN", $00
@src       !byte $DE, $AD, $BE, $EF
@found     !byte 0
@buf_size  = 40
@buf       !fill 40, 0
@name      !text "DIR_READS", $00
...
; =====================================================================
test_dir_chdir
    lda #<@sub                  ; make it (ignore "already exists")
    ldx #>@sub
    ldy #@sub_len
    jsr dos_mkdir
...
@done
    ...
    lda #<@root                 ; back to where the other tests live
    ldx #>@root
    ldy #@root_len
    jsr dos_chdir
    bcs @fail

    lda @others                 ; nothing but . and .. in a new directory
    bne @fail
    lda @dirs
    cmp #2
    bne @fail
...
test_bmx_lasterr
    ...
    stz X16_P6
    jsr bmx_load
    bcc @fail                   ; it "loaded" a file that is not there
    jsr bmx_lasterr
    cmp #BMX_ERR_IO
    bne @fail

    lda #<@bmx                  ; a real one, written here and read back
    sta X16_P0
    lda #>@bmx
    sta X16_P1
    lda #@bmx_len
...
@report
    ...
    ldy #>@name
    jmp t_result
@gone      !text "NOSUCH.BMX"
@gone_len  = 10
@bmx       !text "TESTOUT.BMX"
@bmx_len   = 11
@name      !text "BMX_LASTERR", $00
...
test_str_case_hibyte
    ...
    lda #<@hi                   ; the string keeps its length and its $80
    ldx #>@hi
    jsr str_lower
    cpy #4
    bne @fail
    ...
    cmp #$80
    bne @fail
    lda @hi+3                   ; the tail survived the fold
    cmp #67
    bne @fail

    lda #<@s1                   ; s1 is longer, so it sorts after s2
    ldx #>@s1
    lda #<@s2
    sta X16_P0
    lda #>@s2
...
@report
    ...
    ldy #>@name
    jmp t_result
@hi   !byte 97, 98, $80, 99, 0
@s1   !byte 65, 66, $80, 88, 0
@s2   !byte 65, 66, 0
@name !text "STR_CASE_HIBYTE", 0
...
test_str_append_self
    lda #<@buf                  ; suffix and target are the same string
    sta X16_P0
    lda #>@buf
    sta X16_P1
    lda #<@buf
    ...
    cmp #'c'
    bne @fail
    lda @buf+6                  ; terminated, and nothing beyond it
    bne @fail
    lda @buf+7
    cmp #$5A
    bne @fail
...
test_ring_full_edge
    lda #6
    jsr ring_init
@fill                           ; exactly the guarded pattern a caller
...
@atcap
    ...
    bne @fail                   ; end and underflowed free to $FFFF
...
@report
    ...
    ldy #>@name
    jmp t_result
@name !text "SPRITE_POS_UNSIGNED", 0
...
@report
    ...
    ldy #>@name
    jmp t_result
@stars  !text "*a*a*a*a*a*a*a*a*a*a*a*a*a*a*a*a*a*a*a*a*", 0
@evil   !text "a*a*a*a*b", 0
@aaa    !text "aaaaaaaaaaaaaaaaaaaaaaaa", 0
@star1  !text "*", 0
@empty  !byte 0
@lead   !text "*llo", 0
@hello  !text "hello", 0
@name   !text "STR_PAT_EDGE", 0
