...
.salpha   byte "alpha", 0
.sbravo   byte "bravo", 0
.scharlie byte "charlie", 0
.sdelta   byte "delta", 0
.arr      blk 8, 0
.name     byte "STR_SORT", 0
...
.entry
    ...
    sta X16_P2
    jmp dir_next
.want      byte "TESTDATA.BIN", $00
.src       byte $DE, $AD, $BE, $EF
.found     byte 0
.buf_size  = 40
.buf       blk 40, 0
.name      byte "DIR_READS", $00
...
.report
    ...
    ldy #>.name
    jmp t_result
.gone      byte "NOSUCH.BMX"
.gone_len  = 10
.bmx       byte "TESTOUT.BMX"
.bmx_len   = 11
.name      byte "BMX_LASTERR", $00
...
.report
    ...
    jmp t_result
.buf   byte "abc", 0
       blk 4, $5A             ; a guard the old 256-byte run walked over
.name  byte "STR_APPEND_SELF", 0
