...
str_length__loop
    lda (X16_T0),y
    beq str_length__done
    iny
    bne str_length__loop
str_length__done
    rts
...
str_ncopy__loop
    cpy X16_T2
    beq str_ncopy__cap                    ; hit the cap
    lda (X16_T0),y
    sta (X16_P0),y
    beq str_ncopy__done                   ; copied the NUL
    iny
    bne str_ncopy__loop
str_ncopy__cap
    lda #0
...
str_nappend
    ...
    cpy X16_T3
    bcs str_nappend__toosmall               ; length >= max: no room, leave it be
...
str_nappend__loop
    ...
    beq str_nappend__cap
    lda (X16_P0),y
    sta (X16_T0),y
    beq str_nappend__done
    iny
    bne str_nappend__loop
str_nappend__cap
    lda #0
...
str_compare__loop
    ...
    beq str_compare__s1end
    cmp (X16_P0),y              ; vs string2 char
    bne str_compare__diff
    iny
    bne str_compare__loop
    lda #0                      ; ran the whole page: equal
    rts
str_compare__s1end
    lda (X16_P0),y              ; string1 ended; string2 too?
    beq str_compare__equal
    lda #$FF                    ; string1 is the shorter -> before
    rts
str_compare__diff
    bcs str_compare__greater                ; carry from cmp: set if s1 >= s2
    lda #$FF
    rts
str_compare__greater
    lda #1
    rts
str_compare__equal
    lda #0
