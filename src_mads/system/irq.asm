...
irq_handler
    ...
    and #VERA_IRQ_VSYNC
    beq irq_handler__no_vsync
    inc irq_frame_count         ; the KERNAL acks VSYNC for us
irq_handler__no_vsync

    lda irq_isr
    and #VERA_IRQ_LINE
    beq irq_handler__no_line
    sta VERA_ISR                ; ack FIRST: nobody else will
    lda irq_line_armed
    beq irq_handler__no_line
    jsr irq_call_line
irq_handler__no_line

.if .def X16_USE_IRQ_SPRCOL
    lda irq_isr
    and #VERA_IRQ_SPRCOL
    beq irq_handler__no_sprcol
    sta VERA_ISR                ; ack FIRST: nobody else will
    lda irq_isr
...
.endif
    ...
    lda irq_isr
    and #VERA_IRQ_VSYNC
    bne irq_handler__counted
    lda VERA_ISR
    and #VERA_IRQ_VSYNC
    beq irq_handler__counted
    inc irq_frame_count
irq_handler__counted
    jmp (irq_old_vector)
...
irq_line_install
    ...
    bcs irq_line_install__bit8_set
    trb VERA_IEN
    bra irq_line_install__bit8_done
irq_line_install__bit8_set
    tsb VERA_IEN
irq_line_install__bit8_done
    lda #VERA_IRQ_LINE
...
irq_sprcol_install
    ...
    beq irq_sprcol_install__polling
    lda #1
irq_sprcol_install__polling
...
irq_save_regs
    ldx #31
irq_save_regs__save_r
    lda r0L,x                   ; r0-r15 at $02-$21
    sta irq_zp_buf,x
    dex
    bpl irq_save_regs__save_r
    ldx #15
irq_save_regs__save_p
    lda X16_P0,x                ; the library's parameter/scratch block
    sta irq_zp_buf+32,x
    dex
    bpl irq_save_regs__save_p
    rts

irq_restore_regs
    ldx #31
irq_restore_regs__rest_r
    lda irq_zp_buf,x
    sta r0L,x
    dex
    bpl irq_restore_regs__rest_r
    ldx #15
irq_restore_regs__rest_p
    lda irq_zp_buf+32,x
    sta X16_P0,x
    dex
    bpl irq_restore_regs__rest_p
    rts
