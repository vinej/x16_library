...
zx0_decompress__literals
    jsr zx0_gamma_n                ; literal run length
zx0_decompress__lit_byte
    jsr zx0_getbyte
    sta (X16_P2)
    inc X16_P2
    bne zx0_decompress__lit_dec
    inc X16_P3
zx0_decompress__lit_dec
    jsr zx0_dec_len
    bne zx0_decompress__lit_byte

    jsr zx0_getbit
    bcs zx0_decompress__new_offset

zx0_decompress__last_offset
    jsr zx0_gamma_n                ; match length, offset unchanged
    jsr zx0_copy
    jsr zx0_getbit
    bcc zx0_decompress__literals

zx0_decompress__new_offset
    jsr zx0_gamma_i                ; the offset MSB, inverted gamma (v2)
    lda zx_val+1                ; 256 is the end-of-stream marker
    beq zx0_decompress__not_end
    lda zx_val
    bne zx0_decompress__not_end
    lda X16_P2                  ; done: hand back the output end
    ldx X16_P3
    rts
zx0_decompress__not_end
    ...
    bne zx0_decompress__len_ok
    inc zx_val+1
zx0_decompress__len_ok
    jsr zx0_copy
    jsr zx0_getbit
    bcs zx0_decompress__new_offset
    bra zx0_decompress__literals
...
zx0_copy
    ...
    sbc zx_off+1
    sta X16_T7
zx0_decompress__byte
    lda (X16_T6)
    sta (X16_P2)
    inc X16_T6
    bne zx0_decompress__dst
    inc X16_T7
zx0_decompress__dst
    inc X16_P2
    bne zx0_decompress__count
    inc X16_P3
zx0_decompress__count
    jsr zx0_dec_len
    bne zx0_decompress__byte
    rts

; zx_val -= 1; Z set when it reaches zero (val >= 1 on entry)
zx0_dec_len
    lda zx_val
    bne zx0_decompress__lo
    dec zx_val+1
zx0_decompress__lo
    dec zx_val
    lda zx_val
...
zx0_gamma
    ...
    sta zx_val
    stz zx_val+1
zx0_decompress__more
    jsr zx0_getbit
    bcs zx0_decompress__done                   ; a 1 control bit ends the number
    jsr zx0_getbit
    lda #0
    rol                         ; A = the data bit
    eor zx_inv
    ...
    rol zx_val
    rol zx_val+1
    bra zx0_decompress__more
zx0_decompress__done
    rts
...
zx0_getbit
    lda zx_bt
    beq zx0_decompress__stream
    stz zx_bt                   ; backtrack: the offset byte's low bit
    lda zx_last
    lsr
    rts
zx0_decompress__stream
    asl zx_bits
    beq zx0_decompress__refill
    rts
zx0_decompress__refill
    jsr zx0_getbyte
    sec
...
zx0_getbyte
    ...
    sta zx_last
    inc X16_P0
    bne zx0_decompress__gb_ok
    inc X16_P1
zx0_decompress__gb_ok
    lda zx_last
    rts
