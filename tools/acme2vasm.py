...
# it. vasm has the tier ACME means: a '+' lives INSIDE a routine's
# cheap-label zone, so the pre-pass writes @k<N> and the normal @ -> .
# mapping makes it a vasm local of that routine. Spelling it a
# zone-local instead promoted it to a global, and a global ends a vasm
# local scope -- which cut gfx4h_hline in half and left its own .outer,
# .row and .col unreachable from the code above them.
#
