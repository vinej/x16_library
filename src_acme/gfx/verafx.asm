...
@ft_pick
    bcc @ft_v0_left
    jsr .b_x_a_y                ; v1 left: B in X at x1, A in Y at x0
    lda tri_x1
    sta fxt_px
    lda tri_x1+1
    sta fxt_px+1
    ...
    bra @ft_run
@ft_v0_left
    jsr .a_x_b_y                ; v0 left: A in X at x0, B in Y at x1
    lda tri_x0
    sta fxt_px
    lda tri_x0+1
    sta fxt_px+1
...
.b_x_a_y                        ; edge B (fxs) -> X slot, edge A -> Y/X2
    lda fxs_el
    sta fxt_xl
    lda fxs_eh
    sta fxt_xh
    ...
    sta fxt_yh
    rts
.a_x_b_y                        ; edge A -> X slot, edge B (fxs) -> Y/X2
    lda fxt_a_l
    sta fxt_xl
    lda fxt_a_h
    sta fxt_xh
...
; Both fx_line and fx_triangle divide through .udiv24 and find their
; framebuffer start through .pix_addr, so these are emitted whenever
