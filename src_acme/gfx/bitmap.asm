...
;
; .addr8 -- the 17-bit framebuffer address of (gb8_x, gb8_y) -> the
; port named by .ld0/.ld1, with INC_1. y*320 = (y<<8) + (y<<6).
; ---------------------------------------------------------------------
.addr8
...
.ld0                            ; port 0 <- the address, INC_1
	lda #VERA_CTRL_ADDRSEL
	trb VERA_CTRL
	bra .ldgo
.ld1                            ; port 1 <- the address, INC_1
	lda #VERA_CTRL_ADDRSEL
	tsb VERA_CTRL
.ldgo
	lda gb8_t
