...
;   -- audio/noteconvert.s, `lda bas2midi,x` inside .proc notecon_midi2bas
;
;   psg_getatten (ar_psg_getatten) loads the attenuation into .A, then
;   RESTORE_BANK overwrites .A with the saved RAM bank, and only then
;   copies to .X -- so it returns the RAM bank that was current before
;   the call. psg_getpan does its `tax` BEFORE the restore and is fine.
;   -- audio/psg.s, .proc psg_getatten
;
