...
!addr d_ptr    = X16_TPTR0       ; operand pointer (shared scratch)
!addr dstr_ptr = X16_TPTR1       ; d_from_str's string pointer (survives the
...
d_from_s32
    ...
    beq .dfr_mag
    sec                          ; negate P0..P3
    lda #0 : sbc X16_P0 : sta X16_P0
    lda #0 : sbc X16_P1 : sta X16_P1
    lda #0 : sbc X16_P2 : sta X16_P2
    lda #0 : sbc X16_P3 : sta X16_P3
.dfr_mag
    lda X16_P0
    ora X16_P1
    ora X16_P2
...
.dfr_nz
    ...
    stz dac_m+2
    stz dac_m+3
    lda X16_P0 : sta dac_m+4
    lda X16_P1 : sta dac_m+5
    lda X16_P2 : sta dac_m+6
    lda X16_P3 : sta dac_m+7
    lda #<-32
    sta dac_e
    lda #>-32
    sta dac_e+1
...
.dto_norm
    ; shift = -dac_e
    sec
    lda #0 : sbc dac_e : sta d_cnt
    lda #0 : sbc dac_e+1 : sta d_cnt+1
...
.dto_sr
    ...
    dex
    bne .dto_sr
    lda d_work    : sta X16_P0   ; low 32 bits are the integer
    lda d_work+1  : sta X16_P1
    lda d_work+2  : sta X16_P2
    lda d_work+3  : sta X16_P3
    lda dac_s
    bpl .dto_pos
    sec                          ; apply the sign
    lda #0 : sbc X16_P0 : sta X16_P0
...
d_cmp
    sta d_ptr
    sty d_ptr+1
    jsr .d_unpack                ; operand -> dac_*
    jsr .d_ac_to_bf              ; ...move it to dbf_*
    lda #<d_ac
    sta d_ptr
    lda #>d_ac
    sta d_ptr+1
...
.dcm_nz
    ...
    bra .dcm_gt                  ; a>=0, b<0
.dcm_same
    jsr .d_mag_cmp               ; A = -1/0/1 by |a| vs |b|
    tax
    lda dac_s
    bpl .dcm_done                ; positive: order as computed
...
; alignment become a sticky bit that feeds .d_pack's rounding. Near-total
...
.d_add_common
    ...
    cmp #D_INF
    bne .dad_packac              ; inf + finite -> dac
    lda dac_s
    eor dbf_s
    bmi .dad_nan                 ; inf + (-inf) -> NaN
    bpl .dad_packac              ; inf + inf -> dac
.dad_acfin
    lda dbf_c
    cmp #D_INF
...
.dad_addm
    ...
    clc
    lda dac_m   : adc dbf_m   : sta dac_m
    lda dac_m+1 : adc dbf_m+1 : sta dac_m+1
    lda dac_m+2 : adc dbf_m+2 : sta dac_m+2
    lda dac_m+3 : adc dbf_m+3 : sta dac_m+3
    lda dac_m+4 : adc dbf_m+4 : sta dac_m+4
    lda dac_m+5 : adc dbf_m+5 : sta dac_m+5
    lda dac_m+6 : adc dbf_m+6 : sta dac_m+6
    lda dac_m+7 : adc dbf_m+7 : sta dac_m+7
    bcc .dad_pack
    ...
    sta d_sticky
.dad_c0
    ror dac_m+7 : ror dac_m+6 : ror dac_m+5 : ror dac_m+4
    ror dac_m+3 : ror dac_m+2 : ror dac_m+1 : ror dac_m
    lda #$80
    ora dac_m+7
    sta dac_m+7
    inc dac_e
...
.dad_subgo
    ...
    lda dbf_m   : sbc dac_m   : sta dac_m
    lda dbf_m+1 : sbc dac_m+1 : sta dac_m+1
    lda dbf_m+2 : sbc dac_m+2 : sta dac_m+2
    lda dbf_m+3 : sbc dac_m+3 : sta dac_m+3
    lda dbf_m+4 : sbc dac_m+4 : sta dac_m+4
    lda dbf_m+5 : sbc dac_m+5 : sta dac_m+5
    lda dbf_m+6 : sbc dac_m+6 : sta dac_m+6
    lda dbf_m+7 : sbc dac_m+7 : sta dac_m+7
    lda dbf_s
    sta dac_s
    bra .dad_subnorm
.dad_abig
    sec                          ; dac - dbf, sign = dac_s (unchanged)
    lda dac_m   : sbc dbf_m   : sta dac_m
    lda dac_m+1 : sbc dbf_m+1 : sta dac_m+1
    lda dac_m+2 : sbc dbf_m+2 : sta dac_m+2
    lda dac_m+3 : sbc dbf_m+3 : sta dac_m+3
    lda dac_m+4 : sbc dbf_m+4 : sta dac_m+4
    lda dac_m+5 : sbc dbf_m+5 : sta dac_m+5
    lda dac_m+6 : sbc dbf_m+6 : sta dac_m+6
    lda dac_m+7 : sbc dbf_m+7 : sta dac_m+7
.dad_subnorm
    jsr .d_norm                  ; renormalise (may be a big left shift)
    jmp .d_pack
.dad_cancel
...
; swap dac_* <-> dbf_*
.d_swap_ab
    ldx dac_c : lda dbf_c : sta dac_c : stx dbf_c
    ldx dac_s : lda dbf_s : sta dac_s : stx dbf_s
    ldx dac_e : lda dbf_e : sta dac_e : stx dbf_e
    ldx dac_e+1 : lda dbf_e+1 : sta dac_e+1 : stx dbf_e+1
    ldy #7
.dsw_m
    ldx dac_m,y
    lda dbf_m,y
...
.dml_zh
    ...
    ldx #64
.dml_loop
    lsr dbf_m+7 : ror dbf_m+6 : ror dbf_m+5 : ror dbf_m+4
    ror dbf_m+3 : ror dbf_m+2 : ror dbf_m+1 : ror dbf_m
    bcc .dml_noadd
    clc
    lda d_prod+8  : adc dac_m   : sta d_prod+8
    lda d_prod+9  : adc dac_m+1 : sta d_prod+9
    lda d_prod+10 : adc dac_m+2 : sta d_prod+10
    lda d_prod+11 : adc dac_m+3 : sta d_prod+11
    lda d_prod+12 : adc dac_m+4 : sta d_prod+12
    lda d_prod+13 : adc dac_m+5 : sta d_prod+13
    lda d_prod+14 : adc dac_m+6 : sta d_prod+14
    lda d_prod+15 : adc dac_m+7
    bra .dml_rot
.dml_noadd
    lda d_prod+15
.dml_rot
...
.dml_norm
    ...
    lda d_prod+15
    bmi .dml_top
    asl d_prod : rol d_prod+1 : rol d_prod+2 : rol d_prod+3
    rol d_prod+4 : rol d_prod+5 : rol d_prod+6 : rol d_prod+7
    rol d_prod+8 : rol d_prod+9 : rol d_prod+10 : rol d_prod+11
    rol d_prod+12 : rol d_prod+13 : rol d_prod+14 : rol d_prod+15
    ldy #63
.dml_top
    sty d_t0
    lda d_prod+8  : sta dac_m
    lda d_prod+9  : sta dac_m+1
    lda d_prod+10 : sta dac_m+2
    lda d_prod+11 : sta dac_m+3
    lda d_prod+12 : sta dac_m+4
    lda d_prod+13 : sta dac_m+5
    lda d_prod+14 : sta dac_m+6
    lda d_prod+15 : sta dac_m+7
    lda d_prod : ora d_prod+1 : ora d_prod+2 : ora d_prod+3
    ora d_prod+4 : ora d_prod+5 : ora d_prod+6 : ora d_prod+7
    beq .dml_nost
    lda #1
    sta d_sticky
.dml_nost
    clc                          ; exp = dac_e + dbf_e + adjust
    lda dac_e : adc dbf_e : sta dac_e
    lda dac_e+1 : adc dbf_e+1 : sta dac_e+1
    clc
    lda dac_e : adc d_t0 : sta dac_e
    lda dac_e+1 : adc #0 : sta dac_e+1
    lda d_rsign
    sta dac_s
    lda #D_NORM
    sta dac_c
...
d_div
    ...
    sty d_ptr+1
    jsr .d_unpack
    jsr .d_ac_to_bf              ; divisor -> dbf_*
    lda #<d_ac
    sta d_ptr
    lda #>d_ac
    sta d_ptr+1
    jsr .d_unpack               ; dividend (d_ac) -> dac_*
    stz d_sticky
    lda dac_s
    eor dbf_s
    and #$80
...
.ddv_div
    ...
    lda dac_m   : sta d_prod+8
    lda dac_m+1 : sta d_prod+9
    lda dac_m+2 : sta d_prod+10
    lda dac_m+3 : sta d_prod+11
    lda dac_m+4 : sta d_prod+12
    lda dac_m+5 : sta d_prod+13
    lda dac_m+6 : sta d_prod+14
    lda dac_m+7 : sta d_prod+15
    lda #0
    ldx #7
.ddv_zl
    sta d_prod,x
    dex
    bpl .ddv_zl
    lsr d_prod+15 : ror d_prod+14 : ror d_prod+13 : ror d_prod+12
    ror d_prod+11 : ror d_prod+10 : ror d_prod+9 : ror d_prod+8
    ror d_prod+7 : ror d_prod+6 : ror d_prod+5 : ror d_prod+4
    ror d_prod+3 : ror d_prod+2 : ror d_prod+1 : ror d_prod
    lda d_prod+8  : sta d_rem            ; rem = high half
    lda d_prod+9  : sta d_rem+1
    lda d_prod+10 : sta d_rem+2
    lda d_prod+11 : sta d_rem+3
    lda d_prod+12 : sta d_rem+4
    lda d_prod+13 : sta d_rem+5
    lda d_prod+14 : sta d_rem+6
    lda d_prod+15 : sta d_rem+7
    ldx #64
.ddv_loop
    asl d_prod
    rol d_prod+1
    ...
    rol d_rem+6
    rol d_rem+7
    bcs .ddv_sub                 ; bit 64 set -> definitely subtract
    sec
    lda d_rem   : sbc dbf_m   : sta d_diff
    lda d_rem+1 : sbc dbf_m+1 : sta d_diff+1
    lda d_rem+2 : sbc dbf_m+2 : sta d_diff+2
    lda d_rem+3 : sbc dbf_m+3 : sta d_diff+3
    lda d_rem+4 : sbc dbf_m+4 : sta d_diff+4
    lda d_rem+5 : sbc dbf_m+5 : sta d_diff+5
    lda d_rem+6 : sbc dbf_m+6 : sta d_diff+6
    lda d_rem+7 : sbc dbf_m+7 : sta d_diff+7
    bcc .ddv_noq                 ; borrow -> rem < divisor
    bra .ddv_setq
.ddv_sub
    sec
    lda d_rem   : sbc dbf_m   : sta d_diff
    ...
    lda d_rem+7 : sbc dbf_m+7 : sta d_diff+7
.ddv_setq
    lda d_diff   : sta d_rem
    lda d_diff+1 : sta d_rem+1
    lda d_diff+2 : sta d_rem+2
    lda d_diff+3 : sta d_rem+3
    lda d_diff+4 : sta d_rem+4
    lda d_diff+5 : sta d_rem+5
    lda d_diff+6 : sta d_rem+6
    lda d_diff+7 : sta d_rem+7
...
.ddv_noq
    ...
    jmp .ddv_loop
.ddv_fin
    lda d_rem : ora d_rem+1 : ora d_rem+2 : ora d_rem+3
    ora d_rem+4 : ora d_rem+5 : ora d_rem+6 : ora d_rem+7
    beq .ddv_nost
    lda #1
    sta d_sticky
.ddv_nost
    lda d_prod   : sta dac_m
    lda d_prod+1 : sta dac_m+1
    lda d_prod+2 : sta dac_m+2
    lda d_prod+3 : sta dac_m+3
    lda d_prod+4 : sta dac_m+4
    lda d_prod+5 : sta dac_m+5
    lda d_prod+6 : sta dac_m+6
    lda d_prod+7 : sta dac_m+7
    sec                          ; exp = dac_e - dbf_e - 63
    lda dac_e : sbc dbf_e : sta dac_e
    lda dac_e+1 : sbc dbf_e+1 : sta dac_e+1
    sec
    lda dac_e : sbc #63 : sta dac_e
    lda dac_e+1 : sbc #0 : sta dac_e+1
    lda d_rsign
    sta dac_s
    lda #D_NORM
    sta dac_c
    jsr .d_norm                  ; quotient may need one left shift
    jmp .d_pack
...
d_sqrt
    ...
    lsr d_ac+7 : ror d_ac+6 : ror d_ac+5 : ror d_ac+4
    ror d_ac+3 : ror d_ac+2 : ror d_ac+1 : ror d_ac
    clc
    lda d_ac+6
    adc #$F8
    sta d_ac+6
...
.dex_scale
    ...
    lda dac_c
    cmp #D_NORM
    bne .dex_sdone               ; a zero cannot be scaled
    clc
    lda dac_e
    adc d_tn16
    sta dac_e
...
.dln_c1
    cmp #D_ZERO
    bne .dln_c2
    jmp .dln_ninf                ; ln(0) = -inf
.dln_c2
    lda dac_s
    bpl .dln_c3
    jmp .dln_nan                 ; ln(negative) = NaN
.dln_c3
    lda dac_c
    cmp #D_INF
    bne .dln_norm
...
; ---------------------------------------------------------------------
d_sinh
    jsr .d_hyp_exps              ; d_hypa = e^x, d_hypb = e^-x
    lda #<d_hypa
    ldy #>d_hypa
    jsr d_load
    lda #<d_hypb
...
d_from_str
    ...
    stz dstr_exp
    stz dstr_exp+1
    jsr .d_zero                  ; accumulator = 0
    lda #<dstr_acc
    ldy #>dstr_acc
    jsr d_store

    jsr .dstr_peek               ; optional sign
    bcs .dstr_int                ; empty: fall through the phases to scaling
    cmp #'-'
    bne .dstr_ckplus
    inc dstr_neg
    jsr .dstr_next
...
.dstr_eckp
    ...
    jsr .dstr_next
.dstr_edig
    jsr .dstr_peek               ; exponent digits -> dstr_exp
    bcs .dstr_edone
    cmp #'0'
    bcc .dstr_edone
    cmp #'9'+1
...
.dts_fixed
    ...
    lda dts_p                    ; P > 0
    cmp dts_ndig
    bcc .dts_mid                 ; P < ndig -> point in the middle
...
.dun_normal
    ...
    lda d_ub   : sta dac_m
    lda d_ub+1 : sta dac_m+1
    lda d_ub+2 : sta dac_m+2
    lda d_ub+3 : sta dac_m+3
    lda d_ub+4 : sta dac_m+4
    lda d_ub+5 : sta dac_m+5
    lda d_ub+6
    and #$0F
    ...
    stz dac_m+7
    ; << 8
    lda dac_m+6 : sta dac_m+7
    lda dac_m+5 : sta dac_m+6
    lda dac_m+4 : sta dac_m+5
    lda dac_m+3 : sta dac_m+4
    lda dac_m+2 : sta dac_m+3
    lda dac_m+1 : sta dac_m+2
    lda dac_m   : sta dac_m+1
    stz dac_m
    ; << 3
    asl dac_m : rol dac_m+1 : rol dac_m+2 : rol dac_m+3
    rol dac_m+4 : rol dac_m+5 : rol dac_m+6 : rol dac_m+7
    asl dac_m : rol dac_m+1 : rol dac_m+2 : rol dac_m+3
    rol dac_m+4 : rol dac_m+5 : rol dac_m+6 : rol dac_m+7
    asl dac_m : rol dac_m+1 : rol dac_m+2 : rol dac_m+3
    rol dac_m+4 : rol dac_m+5 : rol dac_m+6 : rol dac_m+7
...
.dpk_cpm
    ...
    lda d_t0
    beq .dpk_rounded             ; R = 0: truncate
    lda d_t1
    bne .dpk_up                  ; R=1, S!=0: up
    lda d_work+1                 ; tie: up only if bit 11 (lsb kept) is set
    and #$08
    beq .dpk_rounded
.dpk_up
    clc
    lda d_work+1 : adc #$08 : sta d_work+1
    lda d_work+2 : adc #0 : sta d_work+2
    lda d_work+3 : adc #0 : sta d_work+3
    lda d_work+4 : adc #0 : sta d_work+4
    lda d_work+5 : adc #0 : sta d_work+5
    lda d_work+6 : adc #0 : sta d_work+6
    lda d_work+7 : adc #0 : sta d_work+7
    bcc .dpk_rounded
    ror d_work+7 : ror d_work+6 : ror d_work+5 : ror d_work+4
    ror d_work+3 : ror d_work+2 : ror d_work+1 : ror d_work
    lda #$80
    ora d_work+7
    sta d_work+7
    inc dac_e
...
.dpk_asm
    ...
    lda d_work+1 : sta d_work
    lda d_work+2 : sta d_work+1
    lda d_work+3 : sta d_work+2
    lda d_work+4 : sta d_work+3
    lda d_work+5 : sta d_work+4
    lda d_work+6 : sta d_work+5
    lda d_work+7 : sta d_work+6
    stz d_work+7
    lsr d_work+6 : ror d_work+5 : ror d_work+4 : ror d_work+3
    ror d_work+2 : ror d_work+1 : ror d_work
    lsr d_work+6 : ror d_work+5 : ror d_work+4 : ror d_work+3
    ror d_work+2 : ror d_work+1 : ror d_work
    lsr d_work+6 : ror d_work+5 : ror d_work+4 : ror d_work+3
    ror d_work+2 : ror d_work+1 : ror d_work
    ...
    lda d_work   : sta d_ac
    lda d_work+1 : sta d_ac+1
    lda d_work+2 : sta d_ac+2
    lda d_work+3 : sta d_ac+3
    lda d_work+4 : sta d_ac+4
    lda d_work+5 : sta d_ac+5
